...
Proof.
  ...
  (* exists (typ_arrow A B). eapply typing_app; eauto. *)
  dependent destruction H0; eauto.
Admitted.

Lemma stack_arrow_inversion1 :
...
Lemma papp_progress :
  forall (e1 e2 : trm) (A B : typ),
    rvalue e2 -> typing nil (cons A nil) e1 (typ_arrow A B) ->
    typing nil nil e2 A ->
...
(* value e \/  (exists e', step e e') \/ e is absctration-form *)
Proof.
  intros e A Htyp.
  ...
  - assert (rvalue e2 \/ (exists e', step e2 e')); eauto.
    clear IHHtyp1 IHHtyp2.
    right.
    destruct H.
    + assert (exists e', papp e1 e2 e'). eapply papp_progress; eauto.
      destruct H0. exists x. eapply step_papp; eauto.
      admit. admit.
  ...
  - destruct IHHtyp1; destruct IHHtyp2; eauto.
    + left. admit.
    + right. destruct H1.
      exists (trm_merge e1 x). eapply step_merge_r; eauto.
      admit.
    + right. destruct H0.
