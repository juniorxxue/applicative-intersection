...
Inductive selectorT : type -> type -> Prop :=
| SeT_Arr : forall A B,
    selectorT (Arr A B) B
| SeT_Rcd : forall l A,
    selectorT (Rcd l A) A.

Hint Constructors selectorT : core.
...
(*          (Ann (Lit 1) C) *)
| Pa_Top : forall p A B vl,
    selectorT A B -> toplike B ->
    papp (Ann p A) vl (Ann (Lit 1) B)
...
Ltac solver2 := match goal with
                | SeT : selectorT _ _ |- _ => dependent destruction SeT
                end.
...
Proof.
  ...
  - Case "Toplike".
    dependent destruction Valf. destruct H1;
      dependent destruction H; dependent destruction P2; try solve [solver2; eauto]; eauto.
  - Case "Lam".
    dependent destruction P2; try solve [solver2; eauto]; eauto.
...
Proof.
  ...
  dependent induction P1; intros.
  - Case "Toplike".
    dependent destruction Val. destruct H1;
      dependent destruction H; dependent destruction P2; try solve [solver2; eauto]; eauto.
  - Case "Prj".
    dependent destruction P2; eauto.
  - Case "Mrg L".
...
Proof.
  ...
  - Case "Lit Toplike".
    ...
    + dependent destruction As; eauto. dependent destruction H. auto.
  - Case "Lam".
...
Proof.
  introv As SeT.
  dependent destruction SeT; eauto.
  - dependent destruction As; eauto.
  - dependent destruction As; eauto.
Qed.
...
Proof.
  ...
  - Case "Toplike".
    ...
    * destruct H1.
      ** eexists. dependent destruction Typ. dependent destruction As; eauto.
      ** eexists. dependent destruction Typ. dependent destruction As; eauto.
    * destruct H1.
      ** dependent destruction Typ. dependent destruction As; eauto.
